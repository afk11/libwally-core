...
#define key_size(member) sizeof(((struct ext_key *)0)->member)
     ...
     * with a computed public key present. If set to BIP33_FLAG_KEY_PUBLIC then
                        ...
                        unsigned char *bytes_out, size_t len)
{
    ...
    const bool attempt_slip132 = flags & BIP32_FLAG_SLIP132_PREFIX;
    ...
    if (attempt_slip132 && hdkey->key_versions != NULL) {
...
static int key_unserialize(const unsigned char *bytes, size_t bytes_len,
                          size_t num_versions, const struct version_def* versions[],  
			  struct ext_key *key_out)
{
